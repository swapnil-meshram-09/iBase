...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    elseif (!preg_match("/^[0-9]{10}$/", $hod_contact)) {
        ...
        // Save session and redirect back to student.php to continue to payment
...
#formBox { width: 450px; margin: auto; background:white; padding: 25px; border-radius:15px; box-shadow:0px 0px 10px #aaa; height:580px;}
h2{ padding:0px; margin:0px; margin-bottom:20px;}
...
input, select { padding: 8px; padding-left: 12px; border:none; border-radius:6px; background:#f2f2f2; width:90%; margin-top:10px; margin-left: 10px;}
select,textarea { width: 95%; }
button { margin-top:15px; margin-bottom:0px; padding:12px; background:#007bff; border:none; color:white; border-radius:10px; width:95%; font-size:16px; cursor:pointer; margin-left: 10px;}
...
<input type="text" name="name" value="<?= htmlspecialchars($name) ?>" oninput="onlyChar(this)" pattern="[A-Za-z ]+" placeholder="Enter Student Name" required>

<label><b>Student Contact Number</b></label>
<input type="text" name="contact" value="<?= htmlspecialchars($contact) ?>" oninput="onlyNumber(this)" maxlength="10" inputmode="numeric" pattern="[0-9]{10}" placeholder="Enter 10-digit Contact Number" required>

<label><b>College Name</b></label>
<input type="text" name="college" oninput="onlyChar(this)" pattern="[A-Za-z ]+" placeholder="Enter College Name" required>
...
<input type="text" name="hod_name" oninput="onlyChar(this)" pattern="[A-Za-z ]+" placeholder="Enter HOD Name" required>

<label><b>HOD Contact Number</b></label>
<input type="text" name="hod_contact" oninput="onlyNumber(this)" maxlength="10" inputmode="numeric" pattern="[0-9]{10}" placeholder="Enter 10-digit HOD Contact" required>
